...
fun LoginScreen(state: UiState, onLogin: (String, String) -> Unit, onGuest: () -> Unit, onChangeServer: () -> Unit) {
    ...
    StartFrame(
        title = stringResource(R.string.action_sign_in),
        subtitle = state.baseUrl + (info?.let { "\n" + stringResource(R.string.login_server_info, it.webtrees, it.module) } ?: ""),
        error = state.error,
    ) {
        OutlinedTextField(
            value = user, onValueChange = { user = it }, label = { Text(stringResource(R.string.login_user)) }, singleLine = true,
            keyboardOptions = KeyboardOptions(imeAction = ImeAction.Next), modifier = Modifier.fillMaxWidth(),
        )
        OutlinedTextField(
            value = password, onValueChange = { password = it }, label = { Text(stringResource(R.string.login_password)) }, singleLine = true,
        ...
        Button(
            ...
            Text(if (state.busy) stringResource(R.string.login_busy) else stringResource(R.string.action_sign_in))
        }
        if (info != null && info.trees.isNotEmpty()) {
            TextButton(onClick = onGuest, modifier = Modifier.fillMaxWidth()) { Text(stringResource(R.string.login_guest)) }
        }
        TextButton(onClick = onChangeServer, modifier = Modifier.fillMaxWidth()) { Text(stringResource(R.string.login_other_address)) }
    }
}
...
fun TreesScreen(state: UiState, onChoose: (de.bgghome.webtrees.nativ.api.TreeInfo) -> Unit, onLogout: () -> Unit, onLogin: () -> Unit, onCancel: (() -> Unit)? = null) {
    ...
    StartFrame(
        ...
        if (state.info?.user?.loggedIn == true) {
            ...
            TextButton(onClick = onLogin, modifier = Modifier.fillMaxWidth()) { Text(stringResource(R.string.action_sign_in)) }
        }
    }
}
