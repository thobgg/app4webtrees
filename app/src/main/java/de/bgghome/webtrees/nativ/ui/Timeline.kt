    ...
    detail.spouseFamilies.forEach { family ->
        ...
        family.children.forEach { child ->
            ...
            val label = stringResource(
                when (child.sex) {
                    "M" -> R.string.timeline_birth_son
                    "F" -> R.string.timeline_birth_daughter
                    else -> R.string.timeline_birth_child
                },
                child.name,
            )
    ...
    Row(
        ...
        Column(Modifier.weight(1f)) {
            ...
            if (fact.sources.isNotEmpty()) {
                Text(
                    stringResource(R.string.fact_sources, fact.sources.joinToString("; ") { it.title }),
