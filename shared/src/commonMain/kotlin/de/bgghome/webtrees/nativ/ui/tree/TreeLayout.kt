...
/** Eine Karte im Baum. Koordinaten in dp, linke obere Ecke. */
data class TreeBox(
    val person: Person,
    val x: Float,
    val y: Float,
    ...
    val right get() = x + TreeLayout.BOX_W

    fun contains(px: Float, py: Float) = px >= x && px <= right && py >= y && py <= y + TreeLayout.BOX_H
}
 ...
 * Nachkommen nach unten. Keine Geisterkarten: Verwandte legt man ueber die "+"-Lasche jeder Karte an. Dazu - wie in einer Familienansicht ueblich - die Geschwister der Mittelperson und
...
class TreeLayout private constructor(
    ...
    /** Hat diese Karte eine "+"-Lasche (Verwandte hinzufuegen: Vater, Mutter, Partner, Kind)? Der eine Weg dafuer - Geisterkarten gibt es nicht. */
    fun hasPlus(box: TreeBox): Boolean = canEdit && !box.person.isPrivate
    ...
    companion object {
        ...
        fun build(
            ...
            val builder = Builder(pedigree, siblings, cousins)

            builder.placeDescendants(descendants, 0f, 0, isFocus = true)
            builder.placeAncestors()

            return builder.finish(canEdit)
        }
    }

    private class Builder(pedigree: Pedigree, val siblings: Map<String, List<Sibling>>, val cousins: Boolean) {
        ...
        private fun unitWidth(node: DescendantNode): Float = BOX_W + node.families.count { it.spouse != null } * (SPOUSE_GAP + BOX_W)

        private fun childSlots(node: DescendantNode): Int = node.families.sumOf { it.children.size }

        private fun subtreeWidth(node: DescendantNode): Float {
            var children = node.families.sumOf { family -> family.children.sumOf { subtreeWidth(it).toDouble() } }.toFloat()
            val slots = childSlots(node)
            if (slots > 1) children += (slots - 1) * H_GAP

            return maxOf(unitWidth(node), children)
        }

        fun placeDescendants(node: DescendantNode, left: Float, depth: Int, isFocus: Boolean): TreeBox {
            val span = subtreeWidth(node)
            val y = depth * (BOX_H + V_GAP)
            var x = left + (span - unitWidth(node)) / 2
            ...
            var childrenWidth = node.families.sumOf { family -> family.children.sumOf { subtreeWidth(it).toDouble() } }.toFloat()
            val slots = childSlots(node)
            ...
            node.families.forEachIndexed { index, family ->
                ...
                val childBoxes = family.children.map { child ->
                    placeDescendants(child, childLeft, depth + 1, false).also { childLeft += subtreeWidth(child) + H_GAP }
                }
        ...
        /** Die geladenen Eltern von Kekule-Platz n. */
        private fun parentsOf(n: Int): List<Int> =
            if (ancestors.containsKey(n)) listOf(2 * n, 2 * n + 1).filter(ancestors::containsKey) else emptyList()
        ...
        private fun placeAncestor(n: Int, centerX: Float) {
            ...
            if (n > 1) {
                ...
                boxes += TreeBox(
                    person = ancestors.getValue(n), x = centerX - BOX_W / 2, y = y, ahnen = n,
                    canExpand = n in hasParents && 2 * n !in ancestors && 2 * n + 1 !in ancestors,
                )
            }
        ...
        fun finish(canEdit: Boolean): TreeLayout {
