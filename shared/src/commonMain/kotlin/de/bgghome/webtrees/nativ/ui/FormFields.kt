...
@Composable
internal fun Field(value: String, onChange: (String) -> Unit, label: StringResource, hint: StringResource? = null, minLines: Int = 1) {
    OutlinedTextField(
 ...
 */
@Composable
internal fun PlaceField(value: String, onChange: (String) -> Unit, label: StringResource, suggest: PlaceSuggest?, hint: StringResource? = null) {
...
@Composable
internal fun DateInput(state: DateInputState, label: StringResource) {
    Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
        ...
        if (state.textMode) {
            Field(state.text, { state.text = it }, Res.string.fact_date, hint = Res.string.date_hint)
        } else {
            DatePartRow(state.form.first) { state.form = state.form.copy(first = it) }
            if (state.form.qualifier == DateQualifier.BETWEEN) {
                Text(stringResource(Res.string.date_and), style = MaterialTheme.typography.labelMedium)
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            if (!state.textMode) {
                TextButton(onClick = state::type) { Text(stringResource(Res.string.date_as_text)) }
            } else if (state.canPick) {
                TextButton(onClick = state::pick) { Text(stringResource(Res.string.date_pick)) }
            }
        }
    }
}
...
private fun qualifierLabel(q: DateQualifier): String = stringResource(
    when (q) {
        DateQualifier.EXACT -> Res.string.date_exact
        DateQualifier.ABOUT -> Res.string.date_about
        DateQualifier.BEFORE -> Res.string.date_before
        DateQualifier.AFTER -> Res.string.date_after
        DateQualifier.BETWEEN -> Res.string.date_between
    }
)
...
private fun DatePartRow(part: DatePart, onChange: (DatePart) -> Unit) {
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
        DigitsField(part.day, maxLength = 2, Res.string.date_day, Modifier.weight(0.8f)) { onChange(part.copy(day = it)) }
        MonthField(part.month, Modifier.weight(1.5f)) { onChange(part.copy(month = it)) }
        DigitsField(part.year, maxLength = 4, Res.string.date_year, Modifier.weight(1f)) { onChange(part.copy(year = it)) }
    }
}

@Composable
private fun DigitsField(value: String, maxLength: Int, label: StringResource, modifier: Modifier, onChange: (String) -> Unit) {
    OutlinedTextField(
        value = value,
...
private fun MonthField(month: Int, modifier: Modifier, onChange: (Int) -> Unit) {
    val names = stringArrayResource(Res.array.date_months)
    var open by remember { mutableStateOf(false) }
    ...
    Box(modifier) {
        OutlinedTextField(
            ...
            onValueChange = {},
            readOnly = true,
            label = { Text(stringResource(Res.string.date_month)) },
        ...
        DropdownMenu(expanded = open, onDismissRequest = { open = false }) {
            DropdownMenuItem(text = { Text(stringResource(Res.string.date_month_unknown)) }, onClick = { onChange(0); open = false })
...
private fun DateSummary(form: DateForm) {
    ...
    when {
        gedcom == null -> Text(
            stringResource(Res.string.date_incomplete),
        ...
        gedcom.isNotEmpty() -> {
            ...
            val text = when (form.qualifier) {
                ...
                DateQualifier.ABOUT -> stringResource(Res.string.date_text_about, first)
                DateQualifier.BEFORE -> stringResource(Res.string.date_text_before, first)
                DateQualifier.AFTER -> stringResource(Res.string.date_text_after, first)
                DateQualifier.BETWEEN -> stringResource(Res.string.date_text_between, first, describe(form.second))
            }
...
private fun describe(part: DatePart): String {
    val month = if (part.month in 1..12) stringArrayResource(Res.array.date_months)[part.month - 1] else ""
    val year = part.year.trim()

    return when {
        month.isEmpty() -> year
        part.day.isBlank() -> stringResource(Res.string.date_text_month, month, year)
        else -> stringResource(Res.string.date_text_day, part.day.trim().toInt(), month, year)
    }
}
